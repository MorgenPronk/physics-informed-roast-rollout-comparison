...
\begin{abstract}
Industrial production processes are typically governed by well-established physical laws, but the run-specific information that mechanistic models need (initial latent states, material properties, and startup conditions) is routinely missing or unreliable in routine production records. Physics-informed machine learning (PIML) is a method to make up for this knowledge gap and repair mechanistic models. This study focuses on comparing one major design choice for PIML: \emph{where} the learned neural network attaches to the mechanistic ODE. Using industrial coffee roasting as a positive-control case study (the published heat- and mass-transfer ODE reaches rollout $R^2 \approx 0.93$ when correctly parameterized; the realistic regime is constructed by withholding that parameterization), we compare four PIML repair positions and a matched-input neural baseline as a six-row position spectrum on a 221-roast production cohort. Every learned component is a feedforward MLP, so the only varied factor is the plug-in point relative to the ODE. The positions tested are nowhere (mechanistic model), inside the ODE as one or three learned closures, on top of the rollout as a bounded or unbounded additive correction, or standalone (pure MLP baseline). Three structural findings emerge. First, PIML repair strategies share their hard roasts with one another (per-roast Pearson $r \approx 0.5$--$0.9$) while the matched-input neural baseline has near-orthogonal errors ($r \approx 0$): the two approaches reach a comparable predictive ceiling through structurally different failure profiles. Second, a physics-motivated bound on the residual correction costs more than it buys: the unbounded variant uses 3.7$\times$ fewer parameters and gains $+0.02$ R$^2$ because the bounded variant spends capacity fighting its $\tanh$ constraint. Third, seed stability varies by more than an order of magnitude across PIML strategies (range $0.004$ to $0.117$ R$^2$ across three retraining seeds): out-of-scaffold residual repair is the most reproducible, the highest-R$^2$ in-scaffold strategy is the least. Supporting the structural findings, the four PIML repairs lift the rollout from $R^2 = -0.44$ to between $0.70$ and $0.94$, and the matched-input neural baseline reaches $0.97$ with 705 parameters, 3--20$\times$ fewer than every PIML repair tested. Because the case study sits in PIML's favorable regime (correct physics form, missing only run-specific initialization), this ordering is conservative for the harder cases where PIML is typically deployed.
