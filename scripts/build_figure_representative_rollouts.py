...
def pick_representative_roast(data: dict) -> str:
    ...
    Length is measured on rollout_pred, which every artifact variant carries;
    sanitized public artifacts keep the measured trace only for this roast and
    record it explicitly.
    """
    explicit = data.get("representative_roast_with_measured_trace")
    if explicit:
        return explicit
    mech = data["models"]["mechanistic"]
    longest = max(mech.items(), key=lambda kv: len(kv[1]["rollout_pred"]))
    return longest[0]
